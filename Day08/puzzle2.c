...
static char	*find_S2(char *S1, char **list)
{
	...
	while (ft_strlen(list[word]) != 4 && word < 10)
		word++;
	while (list[word][i])
	{
...
static char	*find_S0(char *S1, char **list)
{
	...
	while (ft_strlen(list[word]) != 3 && word < 10)
		word++;
	while (list[word][i])
	{
		if (list[word][i] != S1[0] && list[word][i] != S1[1])
...
static char	*find_S3(char *S0, char *S1, char *S2, char **list)
{
	...
	while (ft_strlen(list[word]) != 7 && word < 10)
		word++;
	while (list[word][i])
	{
		if (list[word][i] != S1[0] && list[word][i] != S1[1] && 
	...
	while (line < LINES)
	{
		...
		while (num < 4)
		{
			if (ft_strlen(list[line][word]) == 6 && search(list[line][word], S1) && search(list[line][word], S3))
				...
				array[line][num] = 5;
			else if (ft_strlen(list[line][word]) == 6 && search(list[line][word], S2) && search(list[line][word], S3))
				array[line][num] = 6;
...
static void count_part2(int **array)
{
	size_t	line = 0;
	size_t	entry = 0;
	size_t	num_count = 0;
	...
	while (line < LINES)
	{
		...
		{
			if (num_count == 0)
				total += array[line][entry] * 1000;
			if (num_count == 1)
				total += array[line][entry] * 100;
			if (num_count == 2)
				total += array[line][entry] * 10;
			if (num_count == 3)
				total += array[line][entry];
			num_count++;
			entry++;
		}
		num_count = 0;
